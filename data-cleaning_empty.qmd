...
```
(everything other than the Year column is a character... which presents a problem! can be challenging to go from character to integer, easier to go the other direction)


Checking NAs (May 15 naniar test)
```{r}
snowsurvey_csv %>% 
  miss_var_summary()
...
# 10 "-" and  568 "."...
# some have decimal points (i.e., 10.45)
```
...
snowsurvey_csv %>% 
  count(Snow_cover) %>% 
  filter(is.na(as.numeric(Snow_cover))) # seeing what can not be converted to numeric
```
...
# vast majority have "." in most columns, in a handful it appears to mean "0"
```

observers sjeswiet, akataluk-primeau/egirard, sjeswiet/lpollock, egirard appear to use it as a "0"

sometimes "." is used as NA in other columns like observer
...
snowsurvey_fixed %>% filter(Snow_cover == ".") # should return 0 rows
```
...
```{r}
sort(unique(snowsurvey_fixed$Snow_cover))
...
snowsurvey_fixed %>% 
  filter(Snow_cover > 100) # should return 0 rows
```
...
```{r}
snowsurvey_fixed %>% 
  filter(Snow_cover < 0) # should return 0 rows
```

No negative value detected ✅

```{r, eval=FALSE}
# snowsurvey_cleaning <- snowsurvey_csv %>% 
#   mutate(Snow_cover = ifelse(Snow_cover == ".", NA, Snow_cover)) %>% 
#   mutate(Snow_cover = ifelse(Snow_cover == "-", NA, Snow_cover)) %>% 
#   mutate(Snow_cover = ifelse(Snow_cover == "<1", "0", Snow_cover)) %>% 
#   mutate(Snow_cover = ifelse(Snow_cover == "unk", NA, Snow_cover)) %>% 
...
year, site, date as columns currently, along with every species and observations about them
jdate is julian date (day of the year, 1-365)

0s make it so that it's inefficient too  -- note about not seeing a species in a particular area = 0 (if it's site-related, many species will never be seen in certain sites)
...
species_long <- species_csv %>% 
  pivot_longer(
   # cols = 8:357
    ...
    # R is gonna freak out cuz some are numeric, some characters rn
...
species_presence <- species_long %>% 
  # if 0, set to 0 (numeric). else, set to 1 (num)
...
(because species_presence is numeric now)
```{r}
species_presence %>% 
