...
cover_snowsurvey_clean <- snowsurvey_clean_land
...
total_snowsurvey_clean <- cover_snowsurvey_clean %>%
  mutate(
    # When total cover = snow + water + land AND total cover == 100, keep value of total cover. Else, set to NA
    total_clean = case_when(
      ((Total_cover == Snow_cover + Water_cover + Land_cover) & (Total_cover == 100)) ~ Total_cover,
    ...
    # when all 3 columns (4 columns) are 0, set all values to NA
)
