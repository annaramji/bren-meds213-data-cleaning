...
cover_snowsurvey_clean <- cover_snowsurvey_clean %>% 
  mutate(
    Total_cover = case_when(
      (Total_cover = Snow_cover + Water_cover + Land_cover) & (Total_cover == 100) ~ Total_cover,
      NA # not sure about this (I'd rather inspect visually and figure out what's going on)
    ),
    Snow_cover = case_when(
      (Water_cover + Land_cover = Total_cover) & (Snow_cover == NA) ~ 0,
      Snow_cover
    )
